...
void CodeGenFunction::EmitCallAndReturnForThunk(const CGCallee& Callee,
  ...
  if (CurFnInfo->usesInAlloca() || IsUnprototyped) {
    ...
    EmitMustTailThunk(CurGD, AdjustedThisPtr, Callee.getFunctionPointer());
    return;
  }
...
void CodeGenVTables::addVTableComponent(
  ...
  case VTableComponent::CK_DeletingDtorPointer: {
    ...
    fnPtr = llvm::ConstantExpr::getBitCast(fnPtr, elemType);
    builder.add(fnPtr);
    return;
  }
...
void CodeGenVTables::createVTableInitializer(ConstantStructBuilder &builder,
  ...
  for (unsigned i = 0, e = layout.getNumVTables(); i != e; ++i) {
    ...
    for (unsigned i = thisIndex; i != nextIndex; ++i) {
      addVTableComponent(vtableElem, LayoutClass, layout, i, rtti, nextVTableThunkIndex);
    }
