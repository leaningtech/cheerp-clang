...
llvm::Value *CodeGenFunction::EmitHighInt(QualType Ty,
  ...
  llvm::AllocaInst *highint = new llvm::AllocaInst(T, 0, nullptr, "", AllocaInsertPt);
...
void CodeGenFunction::StartFunction(GlobalDecl GD,
  ...
  if (RetTy->isVoidType()) {
    ...
    if (!endsWithReturn(D))
      ++NumReturnExprs;
  } else if (CurFnInfo->getReturnInfo().getKind() == ABIArgInfo::Indirect || IsHighInt(CurFnInfo->getReturnType())) {
