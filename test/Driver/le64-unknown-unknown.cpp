// RUN: %clang --std=gnu++98 -target le64-unknown-unknown -### %s -emit-llvm-only -c 2>&1 | FileCheck %s -check-prefix=ECHO
// RUN: %clang --std=gnu++98 -target le64-unknown-unknown %s -emit-llvm -S -c -o - | FileCheck %s
