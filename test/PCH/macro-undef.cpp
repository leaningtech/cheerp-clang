// RUN: %clang_cc1 -std=gnu++98 -emit-pch -o %t %s
// RUN: %clang_cc1 -std=gnu++98 -fsyntax-only -include-pch %t %s -Wuninitialized -verify
// RUN: %clang_cc1 -std=gnu++98 -fsyntax-only -include-pch %t %s -Wuninitialized -fdiagnostics-parseable-fixits 2>&1 | FileCheck %s
