...
struct S fd1(struct S *a) {
  // CHECK-LABEL: @fd1
  // CHECK: [[RETVAL:%.*]] = alloca %struct._Z1S, align 4
  // CHECK: [[RET:%.*]]    = alloca %struct._Z1S, align 4
  // CHECK: [[CALL:%.*]]   = call i64 @__atomic_load_8(
  // CHECK: [[CAST:%.*]]   = bitcast %struct._Z1S* [[RET]] to i64*
...
void fd2(struct S *a, struct S *b) {
  ...
  // CHECK-NEXT: [[LOAD_A_PTR:%.*]] = load %struct._Z1S** [[A_ADDR]], align 4
  // CHECK-NEXT: [[LOAD_B_PTR:%.*]] = load %struct._Z1S** [[B_ADDR]], align 4
  // CHECK-NEXT: [[COERCED_A:%.*]] = bitcast %struct._Z1S* [[LOAD_A_PTR]] to i8*
...
void fd3(struct S *a, struct S *b, struct S *c) {
  ...
  // CHECK-NEXT: [[LOAD_A_PTR:%.*]] = load %struct._Z1S** [[A_ADDR]], align 4
  // CHECK-NEXT: [[LOAD_B_PTR:%.*]] = load %struct._Z1S** [[B_ADDR]], align 4
  // CHECK-NEXT: [[LOAD_C_PTR:%.*]] = load %struct._Z1S** [[C_ADDR]], align 4
  // CHECK-NEXT: [[COERCED_A:%.*]] = bitcast %struct._Z1S* [[LOAD_A_PTR]] to i8*
  // CHECK-NEXT: [[COERCED_B:%.*]] = bitcast %struct._Z1S* [[LOAD_B_PTR]] to i64*
  // CHECK-NEXT: [[LOAD_B:%.*]] = load i64* [[COERCED_B]], align 4
...
_Bool fd4(struct S *a, struct S *b, struct S *c) {
  ...
  // CHECK-NEXT: [[LOAD_C_PTR:%.*]] = load %struct._Z1S** [[C_ADDR]], align 4
  // CHECK-NEXT: [[COERCED_A:%.*]] = bitcast %struct._Z1S* [[LOAD_A_PTR]] to i8*
  // CHECK-NEXT: [[COERCED_B:%.*]] = bitcast %struct._Z1S* [[LOAD_B_PTR]] to i8*
  // CHECK-NEXT: [[COERCED_C:%.*]] = bitcast %struct._Z1S* [[LOAD_C_PTR]] to i64*
