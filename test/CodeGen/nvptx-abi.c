...
void foo(float4_t x) {
// CHECK-LABEL: @foo
// CHECK: %struct._Z8float4_s* byval align 4 %x
}

void fooN(float4_t x, float4_t y, float4_t z) {
// CHECK-LABEL: @fooN
// CHECK: %struct._Z8float4_s* byval align 4 %x
// CHECK: %struct._Z8float4_s* byval align 4 %y
// CHECK: %struct._Z8float4_s* byval align 4 %z
}
...
void baz(nested_t x) {
// CHECK-LABEL: @baz
// CHECK: %struct._Z8nested_s* byval align 8 %x)
}
