...
int main(int argc, char **argv) {
    struct foo f;
    f.v = argc;
// CHECK: getelementptr inbounds %struct._Z3foo, %struct._Z3foo* %f, i32 0, i32 0
...
// CHECK-NEXT: bitcast i8* {{.*}} to i32*
    gf.v = argc;
// CHECK: bitcast i32* getelementptr inbounds (%struct._Z3foo, %struct._Z3foo* @gf, i32 0, i32 0) to i8*
// CHECK-NEXT: call i8* @llvm.ptr.annotation.p0i8({{.*}}str{{.*}}str{{.*}}i32 8)
    return 0;
}
