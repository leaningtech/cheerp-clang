...
int fn1(int x) {
  ...
  // CHECK: %[[A:.*]] = getelementptr inbounds %struct._Z1A* %[[INITLIST]], i32 0, i32 0
  // CHECK: store i32 %{{.*}}, i32* %[[A]], align 4
  // CHECK: %[[B:.*]] = getelementptr inbounds %struct._Z1A* %[[INITLIST]], i32 0, i32 1
...
int &fn2(int &v) {
  ...
  // CHECK: %[[R:.*]] = getelementptr inbounds %struct._Z1B* %[[INITLIST2:.*]], i32 0, i32 0
