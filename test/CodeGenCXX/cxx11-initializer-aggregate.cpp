...
int fn1(int x) {
  // CHECK: %[[INITLIST:.*]] = alloca %struct._Z1A
  // CHECK: %[[A:.*]] = getelementptr inbounds %struct._Z1A, %struct._Z1A* %[[INITLIST]], i32 0, i32 0
  // CHECK: store i32 %{{.*}}, i32* %[[A]], align 4
  // CHECK: %[[B:.*]] = getelementptr inbounds %struct._Z1A, %struct._Z1A* %[[INITLIST]], i32 0, i32 1
  // CHECK: store i32 5, i32* %[[B]], align 4
  // CHECK: call i32 @_ZN1A1fEv(%struct._Z1A* %[[INITLIST]])
  return A{x, 5}.f();
}
...
int &fn2(int &v) {
  // CHECK: %[[INITLIST2:.*]] = alloca %struct._Z1B, align 8
  // CHECK: %[[R:.*]] = getelementptr inbounds %struct._Z1B, %struct._Z1B* %[[INITLIST2:.*]], i32 0, i32 0
  // CHECK: store i32* %{{.*}}, i32** %[[R]], align 8
  // CHECK: call dereferenceable({{[0-9]+}}) i32* @_ZN1B1fEv(%struct._Z1B* %[[INITLIST2:.*]])
  return B{v}.f();
}
