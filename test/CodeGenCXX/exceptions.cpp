// RUN: %clang_cc1 -std=gnu++98 %s -triple=x86_64-apple-darwin10 -emit-llvm -o - -fcxx-exceptions -fexceptions | FileCheck %s
...
namespace test11 {
  ...
  // CHECK: [[ARRAYBEGIN:%.*]] = getelementptr inbounds [3 x [[A]]]* {{.*}}, i32 0, i32 0
  // CHECK-NEXT: [[ARRAYEND:%.*]] = getelementptr inbounds [[A]]* [[ARRAYBEGIN]], i64 3
  // CHECK-NEXT: br label
