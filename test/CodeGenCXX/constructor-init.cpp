...
namespace InitVTable {
  ...
  // CHECK-LABEL: define void @_ZN10InitVTable1BC2Ev(%struct._ZN10InitVTable1BE* %this) unnamed_addr
  // CHECK:      [[T0:%.*]] = bitcast [[B:%.*]]* [[THIS:%.*]] to i32 (...)***
  // CHECK-NEXT: store i32 (...)** bitcast (i8** getelementptr inbounds ([3 x i8*]* @_ZTVN10InitVTable1BE, i32 0, i32 2) to i32 (...)**), i32 (...)*** [[T0]]
  // CHECK:      [[VTBL:%.*]] = load i32 (...)*** {{%.*}}
  // CHECK:      [[FNP:%.*]] = getelementptr inbounds i32 ([[B]]*)** {{%.*}}, i64 0
  ...
  // CHECK-LABEL: define void @_ZN10InitVTable1BC2Ei(%struct._ZN10InitVTable1BE* %this, i32 %x) unnamed_addr
...
// end properly.
// CHECK-LABEL: define linkonce_odr void @_ZN1XIiEC2ERKS0_(%struct._Z1XIiE* %this, %struct._Z1XIiE* dereferenceable({{[0-9]+}}) %other) unnamed_addr
// CHECK: {{store.*null}}
// CHECK: {{store.*null}}
// CHECK: ret
template<typename T>
