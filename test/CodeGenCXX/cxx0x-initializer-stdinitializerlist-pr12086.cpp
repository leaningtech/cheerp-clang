...
// CHECK-STATIC-BL: @nested = global {{.*}} { {{.*}} bitcast ([3 x {{.*}}]* @_ZGR6nested3 to {{.*}}), i64 3 }, align 8
...
// CHECK-STATIC-BE: @nested = global {{.*}} { {{.*}} bitcast ([3 x {{.*}}]* @_ZGR6nested3 to {{.*}}),
// XFAIL:                           {{.*}} bitcast ({{.*}}* getelementptr (i8* bitcast ([3 x {{.*}}]* @_ZGR6nested3 to i8*), i64 48) to {{.*}}*) }
