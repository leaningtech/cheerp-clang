// RUN: %clang_cc1 %s --std=gnu++98 -triple=thumbv7-apple-ios6.0 -fno-use-cxa-atexit -target-abi apcs-gnu -emit-llvm -o - -fexceptions | FileCheck %s
...
namespace test6 {
  ...
  void test(A *a) {
    ...
    // CHECK:      [[T0:%.*]] = getelementptr [[A]]* [[V]], i32 0, i32 0
    // CHECK-NEXT: [[T1:%.*]] = load i32 (...)*** [[T0]]
    // CHECK-NEXT: [[T2:%.*]] = bitcast i32 (...)** [[T1]] to void ([[A]]*)**
    // CHECK-NEXT: [[T3:%.*]] = getelementptr inbounds void ([[A]]*)** [[T2]], i64 1
    // CHECK-NEXT: [[T4:%.*]] = load void ([[A]]*)** [[T3]]
