...
// CHECK-LABEL: define noalias %"struct.\01?T@@"* @"\01?test0@@YAPAUT@@XZ"()
// CHECK:   ret %"struct.\01?T@@"* null

T* test1(V* x) { return &dynamic_cast<T&>(*x); }
// CHECK-LABEL: define %"struct.\01?T@@"* @"\01?test1@@YAPAUT@@PAUV@@@Z"(%"struct.\01?V@@"* %x)
// CHECK:        [[CAST:%.*]] = bitcast %"struct.\01?V@@"* %x to i8*
...
// CHECK-NEXT:   [[RET:%.*]] = bitcast i8* [[CALL]] to %"struct.\01?T@@"*
// CHECK-NEXT:   ret %"struct.\01?T@@"* [[RET]]

T* test2(A* x) { return &dynamic_cast<T&>(*x); }
// CHECK-LABEL: define %"struct.\01?T@@"* @"\01?test2@@YAPAUT@@PAUA@@@Z"(%"struct.\01?A@@"* %x)
// CHECK:        [[CAST:%.*]] = bitcast %"struct.\01?A@@"* %x to i8*
// CHECK-NEXT:   [[VBPTRPTR:%.*]] = getelementptr inbounds %"struct.\01?A@@"* %x, i32 0, i32 0
...
// CHECK-LABEL: define %"struct.\01?T@@"* @"\01?test3@@YAPAUT@@PAUB@@@Z"(%"struct.\01?B@@"* %x)
// CHECK:        [[VOIDP:%.*]] = getelementptr inbounds %"struct.\01?B@@"* %x, i32 0, i32 0, i32 0
...
// CHECK-LABEL: define %"struct.\01?T@@"* @"\01?test4@@YAPAUT@@PAUV@@@Z"(%"struct.\01?V@@"* %x)
// CHECK:        [[CAST:%.*]] = bitcast %"struct.\01?V@@"* %x to i8*
...
// CHECK-LABEL: define %"struct.\01?T@@"* @"\01?test5@@YAPAUT@@PAUA@@@Z"(%"struct.\01?A@@"* %x)
// CHECK:        [[CHECK:%.*]] = icmp eq %"struct.\01?A@@"* %x, null
// CHECK-NEXT:   br i1 [[CHECK]]
// CHECK:        [[VOIDP:%.*]] = bitcast %"struct.\01?A@@"* %x to i8*
// CHECK-NEXT:   [[VBPTRPTR:%.*]] = getelementptr inbounds %"struct.\01?A@@"* %x, i32 0, i32 0
// CHECK-NEXT:   [[VBTBL:%.*]] = load i32** [[VBPTRPTR]], align 4
// CHECK-NEXT:   [[VBOFFP:%.*]] = getelementptr inbounds i32* [[VBTBL]], i32 1
// CHECK-NEXT:   [[VBOFFS:%.*]] = load i32* [[VBOFFP]], align 4
...
// CHECK-NEXT:   [[RES:%.*]] = bitcast i8* [[CALL]] to %"struct.\01?T@@"*
// CHECK-NEXT:   br label
// CHECK:        [[RET:%.*]] = phi %"struct.\01?T@@"*
// CHECK-NEXT:   ret %"struct.\01?T@@"* [[RET]]

T* test6(B* x) { return dynamic_cast<T*>(x); }
// CHECK-LABEL: define %"struct.\01?T@@"* @"\01?test6@@YAPAUT@@PAUB@@@Z"(%"struct.\01?B@@"* %x)
// CHECK:        [[CHECK:%.*]] = icmp eq %"struct.\01?B@@"* %x, null
// CHECK-NEXT:   br i1 [[CHECK]]
// CHECK:        [[CAST:%.*]] = getelementptr inbounds %"struct.\01?B@@"* %x, i32 0, i32 0, i32 0
...
// CHECK-LABEL: define i8* @"\01?test7@@YAPAXPAUV@@@Z"(%"struct.\01?V@@"* %x)
// CHECK:        [[CAST:%.*]] = bitcast %"struct.\01?V@@"* %x to i8*
// CHECK-NEXT:   [[RET:%.*]] = tail call i8* @__RTCastToVoid(i8* [[CAST]])
// CHECK-NEXT:   ret i8* [[RET]]

void* test8(A* x) { return dynamic_cast<void*>(x); }
// CHECK-LABEL: define i8* @"\01?test8@@YAPAXPAUA@@@Z"(%"struct.\01?A@@"* %x)
// CHECK:        [[CHECK:%.*]] = icmp eq %"struct.\01?A@@"* %x, null
// CHECK-NEXT:   br i1 [[CHECK]]
// CHECK:        [[VOIDP:%.*]] = bitcast %"struct.\01?A@@"* %x to i8*
// CHECK-NEXT:   [[VBPTRPTR:%.*]] = getelementptr inbounds %"struct.\01?A@@"* %x, i32 0, i32 0
...
// CHECK-LABEL: define i8* @"\01?test9@@YAPAXPAUB@@@Z"(%"struct.\01?B@@"* %x)
// CHECK:        [[CHECK:%.*]] = icmp eq %"struct.\01?B@@"* %x, null
// CHECK-NEXT:   br i1 [[CHECK]]
// CHECK:        [[CAST:%.*]] = getelementptr inbounds %"struct.\01?B@@"* %x, i32 0, i32 0, i32 0
// CHECK-NEXT:   [[VBPTR:%.*]] = getelementptr inbounds i8* [[CAST]], i32 4
