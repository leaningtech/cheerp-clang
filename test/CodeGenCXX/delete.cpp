...
namespace test4 {
  ...
  void global_delete_virtual(X *xp) {
    ...
    // CHECK:      [[T0:%.*]] = getelementptr [[X:%.*]]* [[XP:%.*]], i32 0, i32 0
    // CHECK-NEXT: [[VTABLETMP1:%.*]] = load i32 (...)*** [[T0]]
    // CHECK-NEXT: [[VTABLETMP2:%.*]] = bitcast i32 (...)** [[VTABLETMP1]] to i64*
    // CHECK-NEXT: [[T0:%.*]] = getelementptr inbounds i64* [[VTABLETMP2]], i64 -2
    ...
    //   and call it.
    // CHECK-NEXT: [[T0:%.*]] = getelementptr [[X:%.*]]* [[XP:%.*]], i32 0, i32 0
    // CHECK-NEXT: [[VTABLETMP1:%.*]] = load i32 (...)*** [[T0]]
